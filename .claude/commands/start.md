...
Выполни старт сессии «ДК Малмыж» строго по шагам (детали — в `CLAUDE.md` §📬 Mailbox check). Порядок жёсткий: **сначала синхронизация (шаги 1–2), потом чтение session-памяти (шаг 5)** — pool #032.
...
3. **Скан входящих:** прочитай файлы в корне `../brain_matrica/mailboxes/DKMalmyzh/from-brain/*.md` (НЕ `DRAFTS/`, НЕ `ARCHIVE/`).
