...
# сессии, чтобы агент стартовал в контексте без ритуала чтения. Только чтение, без
# сети: синхронизацию с origin делает /start, хук её не подменяет.
...
echo
echo "--- Дальше: /start (sync + почта из двух каналов). Канон: .claude/commands/start.md ---"
exit 0
