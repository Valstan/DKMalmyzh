...
export const Posts: CollectionConfig<'posts'> = {
  ...
  fields: [
    ...
    {
      // Видео записи. Поле появилось при поглощении Калинино (D-074): их посты
      // несут ссылки на плеер ВК и mp4, а перенос без такого поля потерял бы
      // видео молча — отчёт сошёлся бы по числам, контента бы не стало (#279).
      // Отдельным полем, а не узлом richText: наш рендерер сложные узлы не
      // рисует, и видео исчезло бы со страницы так же молча.
      name: 'videos',
      type: 'array',
      label: 'Видео',
      labels: { singular: 'Видео', plural: 'Видео' },
      admin: {
        description: 'mp4 — встроенный плеер, ссылка на плеер ВК — встраиваемый кадр, прочее — ссылка.',
      },
      fields: [
        {
          name: 'title',
    ...
    {
      // Провенанс: откуда запись. У импорта из ВК — адрес исходной записи; у
      // перенесённых из Калинино — тот, что хранили они. Без поля происхождение
      // материала восстанавливалось бы только по vkUid руками.
      name: 'sourceUrl',
      type: 'text',
      ...
      admin: {
        position: 'sidebar',
        readOnly: true,
        description: 'Служебное: адрес исходной записи (ВКонтакте).',
      },
    },
    {
      name: 'source',
