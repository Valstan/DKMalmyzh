...
- **Прод:** https://домкультуры.вмалмыже.рф/ — **Бокс 1** (`831d0ce99bdf.vps.myjino.ru`, 92.51.22.114), порт **:3005**. nginx-vhost — `deploy/nginx-dkmalmyzh.conf`, ставится деплой-воркфлоу; внешние 80/443 терминирует прокси Джино (G150: слушаем :80, редирект по `X-Forwarded-Proto`), TLS-сертификат выпускается в панели Джино.
